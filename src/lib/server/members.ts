...
export type PermanentMemberRecord =
  MemberEntitlements & {
    email: string;
...
export async function ensureMemberForApplication({
  ...
  return adminDb.runTransaction(
    async (transaction) => {
      ...
      let memberId =
        application.memberId?.trim() ||
        "";

      if (!memberId) {
        memberId =
          await generateUniqueMemberId(
            transaction,
          );
      }
      ...
      const memberFields:
        MemberEntitlements = {
          memberId,
          memberListingLimit:
            typeof application.memberListingLimit ===
            "number"
              ? application.memberListingLimit
              : 3,
          paidListingCredits:
            typeof application.paidListingCredits ===
            "number"
              ? application.paidListingCredits
              : 0,
          sponsoredPlacementsPurchased:
            typeof application.sponsoredPlacementsPurchased ===
            "number"
      ...
      transaction.update(
        applicationReference,
        {
          ...
          memberIdAssignedAt:
            FieldValue.serverTimestamp(),
          memberIdAssignedByUid:
            actor.uid,
          memberIdAssignedByEmail:
            actor.email,
          updatedAt:
            FieldValue.serverTimestamp(),
        },
...
export async function createManualMember({
  ...
  if (
    ...
    throw new Error(
      "A valid account purpose is required.",
    );
  }
  ...
  if (
    ...
    return {
      memberId:
        existingDocument.id,
      memberListingLimit:
        typeof existing.memberListingLimit ===
        "number"
          ? existing.memberListingLimit
          : 3,
      paidListingCredits:
        typeof existing.paidListingCredits ===
        "number"
          ? existing.paidListingCredits
          : 0,
      sponsoredPlacementsPurchased:
        typeof existing.sponsoredPlacementsPurchased ===
        "number"
          ? existing.sponsoredPlacementsPurchased
          : 0,
      memberStatus:
        String(
          existing.memberStatus ||
          "active",
        ),
    };
  }
...
export async function createSelfRegisteredMember({
  ...
  if (!normalizedAuthUid) {
    throw new Error(
      "A Firebase account UID is required.",
    );
  }
  ...
  if (
    !existingUidSnapshot.empty
  ) {
    throw new Error(
      "This Firebase account is already linked to a membership.",
    );
  }
    ...
    talentSeekerStatus:
      ...
      data.talentSeekerStatus ===
        ...
        : String(
          ...
          : "not_required",
    memberListingLimit:
      typeof data.memberListingLimit ===
      "number"
        ? data.memberListingLimit
        : 3,
    paidListingCredits:
      typeof data.paidListingCredits ===
      "number"
        ? data.paidListingCredits
        : 0,
    sponsoredPlacementsPurchased:
      typeof data.sponsoredPlacementsPurchased ===
      "number"
