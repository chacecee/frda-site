...
export async function createDiscordMemberInvite({
  ...
  const channelId =
    process.env.DISCORD_MEMBER_INVITE_CHANNEL_ID?.trim();

  const memberRoleId =
    process.env.DISCORD_MEMBER_ROLE_ID?.trim();
  ...
  const expiresAtDate = new Date(
    Date.now() + INVITE_MAX_AGE_SECONDS * 1000
  );
